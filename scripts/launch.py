...
# ---- PRE-FLIGHT: DB, LOGS, DATA ----
os.makedirs("logs", exist_ok=True)
os.makedirs("data", exist_ok=True)
if not os.path.exists("data/golden.parquet"):
    import pandas as pd
    df = pd.DataFrame([
    ...
    df.to_parquet("data/golden.parquet")
    ...
    def write(self, data):
        for s in self.streams:
            try:
                ...
                pass
    def flush(self): pass
sys.stdout = TeeLogger(sys.stdout, open(LOGFILE, "a", encoding="utf-8"))
sys.stderr = TeeLogger(sys.stderr, open(LOGFILE, "a", encoding="utf-8"))
